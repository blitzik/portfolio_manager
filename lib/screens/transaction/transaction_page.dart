...
class TransactionPage extends StatefulWidget implements AutoRouteWrapper {
  ...
  Widget wrappedRoute(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<TransactionBlocFactory>().create(project)..add(TransactionLoaded(transaction)),
      child: this,
    );
  }
}
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text('Current holdings: ${widget.project.amount} ${widget.project.coin}'),
                          ...
                          BlocBuilder<TransactionBloc, TransactionState>(
                            builder: (context, state) {
                              ...
                              if (state is! TransactionLoadedSuccessfully) {
                                return const SizedBox(width: 0, height: 0,);
                              }

                              if (state.proceeds.isEmpty) {
                                return const Text('No proceeds');
                              }
                              ...
                              return Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    const Text('Proceeds', style: TextStyle(fontWeight: FontWeight.bold),),
                                    Expanded(
                                      child: ListView.separated(
                                        itemCount: state.proceeds.length,
                                        ...
                                        itemBuilder: (context, index) {
                                          Proceed proceed = state.proceeds[index];
                                          return Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text('Amount sold: ${proceed.amountSold}'),
                                              Row(
                                                children: [
                                                  const Text('Sold for: '),
                                                  MoneyUsd(proceed.value)
                                                ],
                                              ),
                                              Row(
                                                children: [
                                                  const Text('Costs: '),
                                                  MoneyUsd(proceed.costs)
                                                ],
                                              ),
                                              Row(
                                                children: [
                                                  const Text('Realized P/L: '),
                                                  MoneyUsd(proceed.pnl, isColored: true,)
                                                ],
                                              ),
                                            ],
                                          );
